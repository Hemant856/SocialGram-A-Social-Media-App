    ...
    Friendship.findOne({ $or: [{ from_user: from_id, to_user: to_id }, { from_user: to_id, to_user: from_id }] },
        function(error, existing_friendship) {
            ...
            if (existing_friendship) {
                User.findByIdAndUpdate(from_id, { $pull: { friendships: existing_friendship.from_user } }, function(error, data) {
                    if (error) {
                ...
                User.findByIdAndUpdate(to_id, { $pull: { friendships: existing_friendship.to_user } }, function(error, data) {
                    if (error) {
                        console.log('Error in removing the friendship from the user', error);
                        return;
                    }
